...
to work properly.
Version 16 adds support for sending the EV3 NUMERIC and LOGIC messages in additiion to text messages.  This is needed to
to implement the PARAM_TIME, PARAM_DIST and PARAM_ANGLE commands.  This works for the PARAM_TIME command, but it
may not have been necessary to go to the encodeMessage procedure, although it seems to work for NUMERIC and TEXT.  Now
try to revert back to the original one messageGuin I was using.  The problem that I was havig is that I had
two message blocks named EV3-MSG, and the Ev3 software got confused which block to deliver the numberic message to,
"""
...
#         print('-> IN poll_cmd_file(), interval = {}   thread_stop = {}'.format(interval,thread_stop))   #DELETe--for debugging
        ...
        for cmd_line in cmds:                                 # Put all commands on the global commanbd queue
...
#                 break
            ...
            print("-> Command from file is {}".format(cmd_line))
            
            if cmd_line is not None:
                
                dest_ev3_block, cmd = cmd_line.split()
    
                if cmd.upper() == "PARAM_TIME" or cmd.upper() == "PARAM_DIST" or cmd.upper() == "PARAM_ANGLE":
#                     ev3_dest_block = Ev3_msg_box.Param
                    msg_type = "numeric"
                else:
#                     ev3_dest_block = Ev3_msg_box.Main
                    msg_type = "text"
                    
         
            
            cmd_q.put(cmd_line.upper())
...
def send_cmds_to_ev3(ev3, ex_event):
    ...
    The parameter dest_ev3_block is the message block in the EV3 program that will receive this message.  The name
    of the block (destination) has to be encoded into the message.
    """
#     global thread_stop
    ...
    while not ex_event.is_set():
        ...
        try:
            cmd_line = cmd_q.get(block=True, timeout=inter_cmd_wait)                   # This should block the thread if the queue is empty
            print("-> Command from file is {}".format(cmd_line))
            
            if len(cmd_line) == 0 or cmd_line.isspace():   # Skip commands that are blank or only whitespace
                continue
            
            cmd_line_tokens = cmd_line.split()
            
            if len(cmd_line_tokens) < 2:
                print('** Bad command: {}'.format(cmd_line))
                continue
            elif len(cmd_line_tokens) >=2 and cmd_line_tokens[1] == "EXIT":
            ...
            dest_ev3_block = cmd_line_tokens[0]
            cmd = cmd_line_tokens[1]               #This can either be a command (like FWD) or the number from a parameter (like PARAM_TIME)
...
# Figure out if the command/parameter is all 'numeric' or not.  If it is, send as numberic; otherwise, send as 'text'
            ...
            if msg_type == 'number':
                m = ev3_rpi_ctrl_pkg.messageGuin(dest_ev3_block,cmd,msg_type)  #  convert message; select EV3-CMD block to send to
#                 m = ev3_rpi_ctrl_pkg.encodeMessage(ev3_rpi_ctrl_pkg.MessageType.Numeric, 'EV3-PARAM', cmd)
            elif msg_type == 'text':
                m = ev3_rpi_ctrl_pkg.messageGuin(dest_ev3_block,cmd,msg_type)  #  convert message; select EV3-CMD block to send to
#                 m = ev3_rpi_ctrl_pkg.encodeMessage(ev3_rpi_ctrl_pkg.MessageType.Text, 'EV3-CMD', cmd)
                
            print('** Sending to EV3 msg: {}  msg_type: {}  type: {}'.format(cmd,msg_type,type(cmd)))
