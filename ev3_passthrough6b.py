...
Version 6 takes the loop that reads the file and sends commands to the EV3 and breaks it into (1) read file
(2) add to command queue (3) send commands from command queue to the EV3 (as a separate thread)
"""
...
##    poll_cmd_file(ev3, ev3_cmd_filename, wait_period)           # Look for commands in command file and send to EV3
    ...
    poll_cmd_thr = threading.Thread(target=poll_cmd_file, name='Command_Poll', args=(ev3, ev3_cmd_filename, wait_period))
